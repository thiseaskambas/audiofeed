    ...
    *,
    ...
    if settings.llm_provider == "openai":
        script, llm_usage = await asyncio.to_thread(
            _script_openai, plain, language, word_count
        )
    else:
        script, llm_usage = await asyncio.to_thread(
            _script_google, plain, language, word_count, tts_style_prompt
        )

    if settings.tts_provider == "openai":
        tts_usage = await asyncio.to_thread(_tts_openai, script, voice, out_path)
    else:
